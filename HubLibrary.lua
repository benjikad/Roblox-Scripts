...
local Utility = {}
local Objects = {}
function BL:DraggingEnabled(frame, parent)
...
function BL.CreateLib(LName, themeList)
    ...
    local selectedTab 
    LName = LName or "Library"
    table.insert(BL, LName)
    for i,v in pairs(game.CoreGui:GetChildren()) do
        if v:IsA("ScreenGui") and v.Name == LName then
            v:Destroy()
        end
    end
    ...
    local IntroTL = Instance.new("TextLabel")
    local IntroCorner = Instance.new("UICorner", IntroTL)

    local blurFrame = Instance.new("Frame")

    BL:DraggingEnabled(MainHeader, Main)
    ...
    IntroTL.Position = UDim2.new(0,0,0,0)
    IntroTL.Size = UDim2.new(0,0,0,0)
    IntroTL.BackgroundColor3 = themeList.Header
    IntroTL.Name = "IntroTL"
    IntroTL.Parent = Main
    IntroTL.Text = "Welcome"

    IntroCorner.CornerRadius = UDim.new(0, 4)
    IntroCorner.Name = "IntroCorner"
    ...
    function BL:ChangeColor(prope,color)
    ...
    function Tabs:NewTab(tabName)
        ...
        coroutine.wrap(function()
            ...
            function Elements:NewSwitch(bname,tipINf, org_value, callback)
                showLogo = showLogo or true
                local ButtonFunction = {}
                tipINf = tipINf or "Tip: Clicking this nothing will happen!"
                bname = bname or "Click Me!"
