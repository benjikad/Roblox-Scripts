...
local function BenjisHub()
    local BenjisHubSGuiLocation = nil
    local TS = game:GetService("TweenService")

	for i, item in pairs(game.CoreGui:GetChildren()) do
		if item:FindFirstChild("Main") then
			if item:FindFirstChild("Main"):FindFirstChild("MainHeader") then
				item:Destroy()
			end
		end
	end
    ...
    for i, item in pairs(game.CoreGui:GetChildren()) do
        if item:GetAttribute("isBenjisHub") == true then
            BenjisHubSGuiLocation = item
            local closee = BenjisHubSGuiLocation.Main.MainHeader.close
    		local Main = BenjisHubSGuiLocation.Main
    		
    		TS:Create(closee, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {ImageTransparency = 1}):Play()
    		wait()
    		...
    		wait(1)
    		BenjisHubSGuiLocation:Destroy()
        else
            item:SetAttribute("isBenjisHub", false)
        end
    end

    local Version = "1.10.16" -- this is the current version
    ...
    for i, p in pairs(BannedPlayers) do
        local LP = game:GetService("Players").LocalPlayer
        if LP.userId == p then
            LP:Kick("WOW! Just wow. How did you get banned? You have been temp banned for: '1000000' days. Benji's hub shutdown.")
        end
    end
    ...
    local function BHSUTermanal(command)

        if game.CoreGui.SGFT then

        else
            local ScreenGuiForTermanal = Instance.new("ScreenGui")
            local MF = Instance.new("Frame")

            ScreenGuiForTermanal.Name = "SGFT"
        end

        if command == "Start-Up Mode" then

        end
    end

    function Notify(Text, WaitTime, E)
        if Text and WaitTime then
            if E == true then
                local bindable = Instance.new("BindableFunction") -- create a local bindable function
        
                bindable.OnInvoke = function(selected)
                    if selected == "restart" then
                        BenjisHub()
                    elseif selected == "new instance" then
						if Main then
							Main.Parent:Destroy()
						end
						wait(1)
                ...
                game.StarterGui:SetCore("SendNotification",{
                    Title = "Benji's Hub - "..Version;
                    Text = Text;
                    Duration = WaitTime;
                    Callback = bindable;
                    Button1 = "restart";
                    Button2 = "new instance"
                })
            else
                game.StarterGui:SetCore("SendNotification",{
                    Title = "Benji's Hub - "..Version;
    ...
    if checkVersionIsUpToDate() == true then 

		local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/xHeptc/Kavo-UI-Library/main/source.lua"))()
		local Window = Library.CreateLib("Benji's Hub Key - "..Version, "Ocean")

		local Key = Window:NewTab("Key")
		local KeySection = Key:NewSection("Enter a key")
		KeySection:NewTextBox("Key", "Enter key to open Benji's Hub", function(txt)
			if txt == "B2b11882d5Fd6jA8" then
				...
				local VersionLabel = AboutSection:NewLabel("Ruh roh... looks like you need to restart")

				if checkVersionIsUpToDate() == false then
					VersionLabel:UpdateLabel("Version: '"..Version.."' is out of date! We're gonna restart for you.\n  Click the button below to launch a new version.")
				...
				local function ESP(sb, sn, stc, thickness, showteamMates) -- requires info
				...
				local function noclip()
					...
					Credits.MouseButton1Click:Connect(function()
						ScreenGui:Destroy()
						shutdown = true
						game:GetService("Players").LocalPlayer.Character.LowerTorso.CanCollide = true
					end)
				...
				if isAntiCheatLocked == false then
					...
					if game.PlaceId == 527730528 then
						GlobalContentSection:NewTextBox("Speed (Only #)", "Sets the walk speed of your player. No text allowed!", function(txt)
							game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = txt
						end)
				...
				if game.PlaceId == 1234567890 then
					...
					if game.PlaceId == 281489669 then -- Wizard Tycoon
						...
						local GuiHubb = GameSpecificContentSection:NewButton("WizHax Hub", "Fires Wiz Haxs.", function()
						...
						GameSpecificContentSection:NewButton("Other Hub", "idk", function()
						...
						if game.PlaceId == 527730528 then -- Magic Training
							local wandsAmount = 0
							local wandsDetected = 0
							local didFindAny = false

							local GameSpecificContent = Window:NewTab(GameName)
							local GameSpecificContentSection = GameSpecificContent:NewSection(GameName)

							local aa = GameSpecificContentSection:NewLabel("Presets not fired yet.")
							local aaa = GameSpecificContentSection:NewLabel("Wall snipes not fired yet.")
							local aaaa = GameSpecificContentSection:NewLabel("There are "..wandsAmount.." wands.")
							...
							if game.CoreGui:GetAttribute("isRecomendedFiredYet") == true then
								aaa:UpdateLabel("Wall snipes fired.")
							else
								game.CoreGui:SetAttribute("isRecomendedFiredYet", false)
							end
							...
							local button = GameSpecificContentSection:NewButton("Spell Presets", "Uhh... cant explain rn.", function()
								if game.CoreGui:GetAttribute("isFiredYet") == false then
									...
									TextLabel.Position = UDim2.new(0.05, 0, 1, 0)
							...
							GameSpecificContentSection:NewButton("Pickup all wands", "bruh", function()
								if game.Workspace:FindFirstChild("Wand") then
									for i, wand in pairs(game.Workspace:GetChildren()) do
										if wand.Name == "Wand" then
											wand.Handle.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame
										end
									end
								end

								if game.Workspace:FindFirstChild("Elder Wand") then
									for i, wand in pairs(game.Workspace:GetChildren()) do
										if wand.Name == "Elder Wand" then
											wand.Handle.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame
										end
									end
								end
							...
							local function onStep()
								wandsAmount = wandsDetected
								didFindAny = false
								for i, v in pairs(game.Workspace:GetChildren()) do
									if v.Name == "Wand" or v.Name == "Elder Wand" then
										didFindAny = true
										wandsDetected = wandsDetected + 1
										if wandsDetected == 1 then
											aaaa:UpdateLabel("There is 1 wand.")
										else
											aaaa:UpdateLabel("There are "..wandsAmount.." wands.")
										end
									else
										if didFindAny == false then
											wandsDetected = 0
											aaaa:UpdateLabel("There are "..wandsAmount.." wands.")
										end
									end
							...
							if game.PlaceId == 844929123 then -- Plane Crazy fight server
								local GameSpecificContent = Window:NewTab(GameName)
								local GameSpecificContentSection = GameSpecificContent:NewSection(GameName)

								GameSpecificContentSection:NewTextBox("TP tnt to player", "tps tnt the player selected.", function(value)
								...
								GameSpecificContentSection:NewTextBox("TP to Base", "tp to a base", function(value)
								...
								GameSpecificContentSection:NewButton("Remove Barriors", "Removes barriors", function()
									for i, part in pairs(game.Workspace:GetDescendants()) do 
										if part.Name == "BlackListPart" then
											part:Destroy()
										end
									end
								end)
							else
								if game.PlaceId == 136258770 then
									local GameSpecificContent = Window:NewTab(GameName)
									local GameSpecificContentSection = GameSpecificContent:NewSection(GameName)

									GameSpecificContentSection:NewButton("Remove barriors","Really?", function()
										for i, part in pairs(game.Workspace:GetDescendants()) do
											if part.Name == "ForceField" then
												part:Destroy()
											end
										end
									end)

									local isMoneyGet = false

									GameSpecificContentSection:NewButton("Auto get money","Really?", function()
										isMoneyGet = not isMoneyGet
									end)
									...
									RunService.Stepped:Connect(function()
										if isMoneyGet then
											for i, part in pairs(game.Workspace:GetDescendants()) do
												if part.Name == "CollectMoney" then
													local org_pos = part.CFrame
				...
				for i, item in pairs(game.CoreGui:GetChildren()) do
					if item:GetAttribute("isBenjisHub") == false then
					else
						item:SetAttribute("isBenjisHub", true)
						BenjisHubSGuiLocation = item
					end
				end
				...
				local WelcomeF = Instance.new("Frame", Main)
				local uic = Instance.new("UICorner", WelcomeF)
				WelcomeF.Size = UDim2.new(1,0,1,0)
				WelcomeF.Position = UDim2.new(0,0,0,0)
				WelcomeF.BackgroundColor3 = Color3.fromRGB(20, 30, 55)

				local WelcomeTL = Instance.new("TextLabel", WelcomeF)
				local uic2 = Instance.new("UICorner", WelcomeTL)
				WelcomeTL.Size = UDim2.new(0.75,0,0.25,0)
				WelcomeTL.Position = UDim2.new(0.125,0,0.125,0)
				WelcomeTL.BackgroundColor3 = Color3.fromRGB(30, 40, 65)
				WelcomeTL.Text = "Benji's Hub\nWelcome, "..game:GetService("Players").LocalPlayer.Name.."!"
				WelcomeTL.TextScaled = true
				WelcomeTL.TextColor3 = Color3.fromRGB(255,255,255)

				wait(5)

				TS:Create(WelcomeF, TweenInfo.new(0.5), {Size = UDim2.new(1,0,0,0)}):Play()
			end
		end)
	else
		Notify("Benji's Hub is out of date. Would you like to restart or launch a new instance?", 9999999999999999999999999999, true)
		wait(5)
		script:Destroy()
    end
end
